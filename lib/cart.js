...
export function setCheckoutItems(items) {
  if (typeof window === "undefined") return;
  sessionStorage.setItem("fit-theory-checkout-items", JSON.stringify(items));
}

export function getCheckoutItems() {
  if (typeof window === "undefined") return [];
  const items = sessionStorage.getItem("fit-theory-checkout-items");
  return items ? JSON.parse(items) : [];
}

export function clearCheckoutItems() {
  if (typeof window === "undefined") return;
  sessionStorage.removeItem("fit-theory-checkout-items");
}

// Remove specific items from cart (by productId + size + color)
...
export function mergeGuestCart() {
  if (typeof window === "undefined") return;
  const guestKey = "fit-theory-cart-guest";
  ...
  for (const guestItem of guestItems) {
    const existing = userCart.find(
      (item) => item.productId === guestItem.productId && item.size === guestItem.size && item.color === guestItem.color
    );
    if (existing) {
